...
- [x] **SYNC-03**: Offline writes queue in PendingOperationEntity, auto-sync via WorkManager when online
- [x] **SYNC-04**: User sees "pending sync" indicator when offline mutations are queued
...
- [x] **CART-05**: Cart shows subtotal, item count, and per-line totals
- [x] **CART-06**: Out-of-stock or deleted products show warning inline
- [x] **CART-07**: Cart badge/count visible on navigation tab
- [x] **CART-08**: Empty cart shows illustration with CTA to browse products

### Checkout

- [x] **CHKT-01**: Firebase Cloud Function creates Stripe PaymentIntent server-side
- [x] **CHKT-02**: Customer sees checkout summary (items, totals, address) before payment
- [x] **CHKT-03**: Stripe PaymentSheet presented for card entry (no custom form)
- [x] **CHKT-04**: On payment success: Order document created in Firestore + Room, cart cleared
- [x] **CHKT-05**: On payment failure: error shown, cart preserved, retry allowed
- [x] **CHKT-06**: Order confirmation screen shows order ID, items, and link to order tracking
...
- [x] **ORDR-10**: Status update triggers FCM notification to customer
